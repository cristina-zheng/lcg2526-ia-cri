...
function meanOfAnArray(arrayInput) {
  ...
  for(let i = 0; i < arrayInput.length; i++) {
    mediaArray += arrayInput[i]; //mediaArray = mediaArray + arrayInput[i]
  }
...
function drawStarSizePlot(arrayDiStelle) {
  ...
  for(let i=0; i < arrayDiStelle.length; i++) {
    ...
    //map --> valore lo mappiamo in un intervallo
...
function draw() {
  ...
  text("mouseX: " + mouseX + ",\
    ...
    text("Stelle valide " + stars_valid.length, 20, height/2);
    ...
    //
    text("Media delle dimensioni " + mediaDimensioni.toFixed(2), 20, height / 2 + 20);
